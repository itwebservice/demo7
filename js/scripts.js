$(document).ready(function () {
  ...
  $(function () {
    ...
    city_lzloading(
      "#hotel_city_filter,#activities_city_filter,#city,#ffrom_city_filter,#fto_city_filter"
    );
  });
  ...
  if ($(".dropdown.selectable").length > 0) {
    $(".dropdown.selectable .dropdown-item").on("click", function () {
      ...
      $(this)
        .parents(".dropdown.selectable")
        .children(".btn-dd")[0].textContent = thisOption;
    });
  }

  $("body").delegate(
    ".c-userBlock .card .st-editProfile",
    "click",
    function () {
...
function error_msg_alert(message, base_url = "") {
  ...
  $.post(
    base_url1,
    { message: message, class_name: class_name },
    function (data) {
...
function display_cart(cart_item_count) {
  ...
  if (typeof Storage !== "undefined") {
    if (localStorage) {
      ...
      var cart_list_arr = JSON.parse(
        window.sessionStorage.getItem("cart_list_arr")
      );
    }
  }

  $.post(
    base_url + "Tours_B2B/cart_data_modal.php",
    { cart_list_arr: cart_list_arr, register_id: register_id },
    function (data) {
      $("#cart_div").html(data);
    }
  );
...
function get_cart_items(cart_list_arr) {
  ...
  $.post(
    base_url + "Tours_B2B/get_cart_items.php",
...
function get_checkoutpage_cart() {
  ...
  $.post(
    base_url + "Tours_B2B/checkout_pages/get_checkoutpage_cart.php",
    { cart_list_arr: cart_list_arr, currency: currency_id },
    function (data) {
...
function get_country_code(country_id, country_code) {
  ...
  $.post(
    base_url + "Tours_B2B/view/get_country_code.php",
...
function get_select_hotel(city_id, hotel_id, check_indate, check_outdate) {
  ...
  $.post(
    base_url + "controller/hotel/b2b/search_session_save.php",
    { hotel_array: hotel_array },
    function (data) {
      window.location.href =
        base_url + "Tours_B2B/view/hotel/hotel-listing.php";
    }
  );
}
...
function get_select_activity(activity_city_id, activities_id, checkDate) {
  ...
  $.post(
    base_url + "controller/b2b_excursion/b2b/search_session_save.php",
    { activity_array: activity_array },
    function (data) {
      window.location.href =
        base_url + "Tours_B2B/view/activities/activities-listing.php";
    }
  );
}
...
function get_select_package(dest_id, tour_id, tour_date) {
  ...
  $.post(
    base_url + "controller/custom_packages/search_session_save.php",
    { tours_array: tours_array },
    function (data) {
      window.location.href =
        base_url + "Tours_B2B/view/tours/tours-listing.php";
    }
  );
}

function get_select_hotel_guest(
  city_id,
  hotel_id,
  check_indate,
  check_outdate
  ...
  window.location.href =
    base_url + "Tours_B2B/view/hotel_guest/hotel-listing.php?" + url;
}
...
function get_service_tax(result) {
  ...
  taxes_result &&
    taxes_result.filter((tax_rule) => {
      ...
      result &&
        result.forEach((rule) => {
          if (
            parseInt(tax_rule["entry_id"]) === parseInt(rule["entry_id"]) &&
            rule["rule_id"]
          )
  ...
  applied_rules &&
    applied_rules.map((rule) => {
      var tax_data = taxes_result.find(
        (entry_id_tax) => entry_id_tax["entry_id"] === rule["entry_id"]
      );
  ...
  if (type === "hotel") {
    ...
    $.post(
      ...
      function (data) {
        ...
        try {
          var cart_list_arr1 = JSON.parse(
            localStorage.getItem("cart_list_arr")
          );
    ...
    try {
      ...
      $.post(
        base_url + "controller/b2b_customer/update_cart.php",
        { register_id: register_id, cart_list_arr: cart_list_arr1 },
        function (data) {
...
function cart_error_pop() {
  ...
  error_msg_alert(
    'Please <a target="_blank" href="' +
      base_url +
      'view/b2b_customer/registration/index.php">Register</a> or <a target="_blank" href="' +
      base_url +
      'Tours_B2B/login.php">Login</a> here for the bookings.'
  );
}
...
function get_other_rules(travel_type, invoice_date) {
  ...
  taxes = cache_rules.filter(
    (el) =>
      el.entry_id !== "" &&
      el.rule_id === undefined &&
      el.entry_id !== undefined &&
      el.currency_id === undefined
  );
  ...
  tax_rules = cache_rules.filter(
    (el) => el.rule_id !== "" && el.rule_id !== undefined
  );
...
function get_tours_data(dest_id, type) {
  ...
  if (type == "1" || type == "2") {
    ...
    $.post(
      base_url + "controller/custom_packages/search_session_save.php",
      { tours_array: tours_array, currency: currency },
      function (data) {
        if (type == "1") {
          window.location.href = b2c_base_url + "view/tours/tours-listing.php";
        } else if (type == "2") {
          window.location.href =
            b2c_base_url + "view/group_tours/tours-listing.php";
        }
      }
    );
  } else if (type == "3") {
    ...
    $.post(
      base_url + "controller/hotel/b2c_search_session_save.php",
      { hotel_array: hotel_array },
      function (data) {
    ...
    $.post(
      ...
      function (data) {
        window.location.href =
          b2c_base_url + "view/activities/activities-listing.php";
      }
    );
  } else if (type == "5") {
    var today = new Date();
    ...
    $.post(
      base_url + "controller/b2b_transfer/b2b/search_session_save.php",
      { pick_drop_array: pick_drop_array },
      function (data) {
        window.location.href =
          b2c_base_url + "view/transfer/transfer-listing.php";
      }
    );
  } else if (type == "6") {
    var visa_array = [];
    ...
    $.post(
      base_url + "controller/visa_master/search_session_save.php",
      { visa_array: visa_array },
      function (data) {
    ...
    $.post(
      base_url + "controller/ferry/search_session_save.php",
      { ferry_array: ferry_array },
      function (data) {
